...
void BlockLocalPositionEstimator::publishOdom()
{
	...
	if (PX4_ISFINITE(_x(X_x)) && PX4_ISFINITE(_x(X_y)) && PX4_ISFINITE(_x(X_z)) &&
		...
		_pub_odom.get().position_covariance[_pub_odom.get().POSITION_COVARIANCE_X_VARIANCE] = m_P(X_vx, X_vx);
		_pub_odom.get().position_covariance[_pub_odom.get().POSITION_COVARIANCE_Y_VARIANCE] = m_P(X_vy, X_vy);
		_pub_odom.get().position_covariance[_pub_odom.get().POSITION_COVARIANCE_Z_VARIANCE] = m_P(X_vz, X_vz);

		// unknown orientation covariances
		// TODO: add orientation covariance to vehicle_attitude
		_pub_odom.get().orientation_covariance[_pub_odom.get().ORIENTATION_COVARIANCE_ROLL_VARIANCE] = NAN;
		_pub_odom.get().orientation_covariance[_pub_odom.get().ORIENTATION_COVARIANCE_PITCH_VARIANCE] = NAN;
		_pub_odom.get().orientation_covariance[_pub_odom.get().ORIENTATION_COVARIANCE_YAW_VARIANCE] = NAN;
		...
		_pub_odom.get().velocity_covariance[_pub_odom.get().VELOCITY_COVARIANCE_VX_VARIANCE] = m_P(X_vx, X_vx);
		_pub_odom.get().velocity_covariance[_pub_odom.get().VELOCITY_COVARIANCE_VY_VARIANCE] = m_P(X_vy, X_vy);
		_pub_odom.get().velocity_covariance[_pub_odom.get().VELOCITY_COVARIANCE_VZ_VARIANCE] = m_P(X_vz, X_vz);
